...
window.onload = function () {
  ...
  convert(threatcodeJar.toString(), pipelineJar.toString());
};
...
function showTab(tabId, codeId){
  ...
  for (i = 0; i < tablinks.length; i++) {
    tablinks[i].classList.remove('bg-threatcode-blue');
    tablinks[i].classList.remove('text-threatcode-dark');
  }
  ...
  tab.classList.add('bg-threatcode-blue');
  tab.classList.add('text-threatcode-dark');
}
...
function generateShareLink() {
  ...
  let rule = encodeURIComponent(btoa(threatcodeJar.toString()));
  ...
  ruleShareBtn.classList.toggle("text-threatcode-blue");
...
function copyQuery() {
  ...
  queryCopyBtn.classList.toggle("text-threatcode-blue");
...
function convert(threatcodeRule, customPipeline) {
  let queryCode = document.getElementById("query-code");
  ...
  xhr.open("post", window.location.origin + "/threatcode", true);
